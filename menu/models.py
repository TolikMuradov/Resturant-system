...
class Order(models.Model):
    ...
    def parsed_items(self):
        """
        JSONField'den ürünlerin isim ve adetlerini ayrıştırır.
        """
        try:
            return [
                {"name": item.get("name", "Ürün Adı Eksik"), "quantity": item.get("quantity", 1)}
                for item in self.items
            ]
        except Exception as e:
            return [{"name": "Hatalı Ürün Verisi", "quantity": 0}]
